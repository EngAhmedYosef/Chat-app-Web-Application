...
<head>
    ...
    <title><?= $translations['Program Specification'] ?></title>
    <style>
        body {
...
<body>
        ...
        if ($_GET['new'] == 'IS' || $_GET['new'] == 'CS') {
            ...
            if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['save'])) {
                ...
                if (!empty($council_committee) && !empty($reference_no) && !empty($date)) {
                    ...
                    $sql = "INSERT INTO specification_approval (user_id, council_committee, reference_no, date, program_id) 
            VALUES ('$user_id', '$council_committee', '$reference_no', '$date', '$program_id')";
                ...
                header("location: Program_Specification_Specification_Approval_Data.php?new=" . $program_name);
                exit;
            }
            ...
            if (isset($_POST['homepage'])) {
                ...
                // unset($_SESSION['specification_new']);
                // unset($_SESSION['Identification_new']);
                // unset($_SESSION['mission_new']);
                // unset($_SESSION['admission_new']);
                // unset($_SESSION['major_tracks']);
                // unset($_SESSION['exit_points']);
                // unset($_SESSION['administrative_new']);
                // unset($_SESSION['facilities_new']);
                // unset($_SESSION['assurance_new']);
                // unset($_SESSION['annual_approval_new']);
                // unset($_SESSION['curriculum_1_new']);
                // unset($_SESSION['curriculum_2_new']);
                // unset($_SESSION['curriculum_3_new']);
                session_unset();
    ...
    ?>
                ...
                <div class="main">
                    ...
                    <form method="post" id="coursesForm">
                        ...
                        <div id="programs" class="form-container">
                            <h2><?= $translations['Specification Approval Data'] ?></h2>
                            <table>
                                <tr>
                            ...
                            <div class="navigation-buttons">
                                ...
                                <button type="button" class="nav-button" onclick="handleNavigation('Program_Specification_Program_Quality_Assurance.php?new=<?= $program_name ?>')"><?= $translations['Back'] ?></button>
            ...
            if (isset($_GET['edit']) && ($_GET['edit'] == 'IS' || $_GET['edit'] == 'CS')) {
                ...
                if ($edit_item) {
                    $query = mysqli_query($conn, "SELECT * FROM specification_approval WHERE id = '$edit_item' AND user_id = '$user_id'");
                ...
                if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update'])) {
                    ...
                    if (!empty($council_committee) && !empty($reference_no) && !empty($date)) {
                        ...
                        $update_sql = "UPDATE specification_approval SET council_committee = '$council_committee', reference_no = '$reference_no', date = '$date' WHERE id = '$edit_item' AND user_id = '$user_id'";
                    ...
                    header("Location: Program_Specification_Specification_Approval_Data.php?edit=" . $program_name);
                    exit;
                }
                ...
                if (isset($_POST['homepage'])) {
                    $user_id = $_SESSION['user_id'];
                    $lang = $_SESSION['lang'];
                    // unset($_SESSION['specification_edit']);
                    // unset($_SESSION['program_specification_identification_edit']);
                    // unset($_SESSION['mission_edit']);
                    // unset($_SESSION['admission_edit']);
                    // unset($_SESSION['major_tracks']);
                    // unset($_SESSION['exit_points']);

                    // unset($_SESSION['administrative_edit']);

                    // unset($_SESSION['facilities_edit']);
                    // unset($_SESSION['assurance_edit']);

                    // unset($_SESSION['annual_approval_edit']);
                    ...
                    <div class="main">
                        ...
                        <form method="post" id="coursesForm">
                            ...
                            <select name="edit_item" id="editSelect" style="width: 100px; height: 40px; border-radius: 5px;">
                                ...
                                $query = "SELECT * FROM specification_approval WHERE program_id = '$program_id' AND user_id = '$user_id'";
                            ...
                            <div id="programs" class="form-container">
                                ...
                                <table>
                                    <tr>
                                        <th><?= $translations['COUNCIL / COMMITTEE'] ?></th>
                                        <td><input type="text" name="council_committee" value="<?= htmlspecialchars($_SESSION['annual_approval_edit']['council_committee'] ?? '') ?>" placeholder="Enter Council/Committee"></td>
                                    </tr>
                                    <tr>
                                        <th><?= $translations['REFERENCE NO'] ?>.</th>
                                        <td><input type="text" name="reference_no" value="<?= htmlspecialchars($_SESSION['annual_approval_edit']['reference_no'] ?? '') ?>" placeholder="Enter Reference No."></td>
                                    </tr>
                                    <tr>
                                        <th><?= $translations['DATE'] ?>:</th>
                                ...
                                <div class="navigation-buttons">
                                    ...
                                    <button type="button" class="nav-button" onclick="handleNavigation('Program_Specification_Program_Quality_Assurance.php?edit=<?= $program_name ?>')"><?= $translations['Back'] ?></button>
                                    <button type="submit" name="homepage" class="nav-button"><?= $translations['Home Page'] ?></button>
                                </div>
                            </div>
                        </form>
